...
def _build_drilldown_queryset(request):
    ...
    qs = Cita.objects.select_related('id_paciente', 'id_especialista__id_usuario').filter(
...
def dashboard_operativo(request):
    ...
    por_especialista = (
        citas_hoy.values('id_especialista', 'id_especialista__id_usuario__nombres', 'id_especialista__id_usuario__apellidos')
        .annotate(
            total=Count('id_cita'),
    ...
    results = [
        {
            ...
            'especialista': f'{c.id_especialista.id_usuario.apellidos}, {c.id_especialista.id_usuario.nombres}',
        }
        for c in items
    ]
