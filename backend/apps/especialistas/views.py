...
class EspecialistaViewSet(viewsets.ModelViewSet):
    queryset = Especialista.objects.select_related('id_usuario').all()
    serializer_class = EspecialistaSerializer
    permission_classes = [IsAuthenticated, IsAdministrativoOrAdmin]
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    filterset_fields = ['activo', 'especialidad']
    search_fields = ['id_usuario__nombres', 'id_usuario__apellidos', 'registro_profesional', 'especialidad']
    ordering_fields = ['id_usuario__apellidos', 'especialidad', 'fecha_creacion']
